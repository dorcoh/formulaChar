...
def run(filename, executable, timeout_sec):
	""" run solver """
	runstring = 'bash ' + str(executable) + ' ' + str(filename)
	lines = list()
...
def main(argv):
	...
	# solvers list
	solvers = ['wrapperLBD.sh','wrapperClause.sh']
	solversDict = {'wrapperLBD.sh': 'LBD_Cut_5', 'wrapperClause.sh': 'Clause_Size_12'}

	# handle arguments
	try:
		opts, args = getopt.getopt(argv, "hrfp:t:l:")
	except getopt.GetoptError:
		print 'exp3.py -p <path> -t <timeout>'
		sys.exit(2)
